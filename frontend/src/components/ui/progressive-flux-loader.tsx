...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  ...
  if (reduced) {
    return (
      <div aria-hidden className={base} style={{ fontFamily: 'var(--font-vt)' }}>
        {label}
      </div>
    );
  }
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        aria-hidden
        className={base}
        style={{ transformStyle: "preserve-3d", fontFamily: 'var(--font-vt)' }}
...
export function ProgressiveFluxLoader({
  ...
  duration = 4, // Shorter duration since API is quick
  loop = true,
  showLabel = true,
